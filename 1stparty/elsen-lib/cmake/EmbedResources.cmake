function (elsen_embed_resources)
    set (
        oneValueArgs
        TARGET
    )
    ...
    cmake_parse_arguments (
        ELSEN "" "${oneValueArgs}" "${multiValueArgs}" ${ARGN}
    )
    ...
    function (embedded_resource input_file as_text resulting_file resource_name)
        if (EXISTS "${input_file}")
            ...
            message (FATAL_ERROR "[elsen] Could not find resource \"${input_file}\"")
        endif ()
        ...
        message (STATUS "[elsen] Generating ${output_file} from ${input_file}")
    ...
    list (LENGTH ELSEN_RESOURCES ELSEN_RESOURCES_COUNT)
    set (counter 1)
    foreach (resource IN LISTS ELSEN_RESOURCES)
        if (NOT (${resource} STREQUAL "TEXT"))
            if (counter LESS ELSEN_RESOURCES_COUNT)
                list (GET ELSEN_RESOURCES ${counter} next_value)
    ...
    add_library (
        ${ELSEN_TARGET}_resources STATIC
        ${RESOURCE_SOURCES}
    )
    set_property (TARGET ${ELSEN_TARGET}_resources PROPERTY POSITION_INDEPENDENT_CODE ON)

    target_include_directories (
        ${ELSEN_TARGET}_resources SYSTEM INTERFACE
        ${CMAKE_CURRENT_BINARY_DIR}/resources/
    )

    target_link_libraries (
        ${ELSEN_TARGET} PUBLIC
        ${ELSEN_TARGET}_resources
    )
endfunction ()
