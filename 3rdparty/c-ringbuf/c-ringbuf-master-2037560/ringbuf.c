 ...
 * intended.
 */
void ringbuf_init(struct ringbuf_t *rb)
{
    /* One byte is used for detecting the full condition. */
    rb->size = RINGBUF_CAPACITY_BYTES + 1;
    ringbuf_reset(rb);
}
...
{
    return rb->head >= rb->tail ? ringbuf_capacity(rb) - (rb->head - rb->tail)
                                : rb->tail - rb->head - 1;
}
...
size_t ringbuf_findchr(const struct ringbuf_t *rb, int c, size_t offset)
{
    ...
    return found ? offset + (found - start) : ringbuf_findchr(rb, c, offset + n);
}
